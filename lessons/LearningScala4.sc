object LearningScala4 {
  ...
  val myObjs = ("Pente", "Meia","Trompa", "Avião")

  println("Obj1: " + myObjs._1)
...
// Also, you understand MapReduce now :)
  ...
  //----------------------------------
  //Example map
  val square = myList.map(x => x*x)
  //Example reduce
  val conv = myList.reduce((x: Int,y: Int) => x*y)
  //Example filter
  val noEven = myList.filter((x: Int) => x%2 != 0)

  //IMPORTANT!!
  //Short syntax for functions that receive known parameters
  val noOdd = myList.filter(_%2 == 0)
...
println(archersShip)                              //> Unknown
  ...
  val carBrand = Map("Onix" -> "Chevrolet", "Corsa" -> "Chevrolet", "Voyage" -> "Wolks", "A3" -> "Audi")

  val A3 = util.Try(carBrand("A3")) getOrElse "unknown"

  carBrand.contains("Fiat")
...
// go into more depth - but you've got enough to get through this course for now.
  ...
  def getDivisible(values: List[Int],div: Int): List[Int] = {
    var divisible = List[Int]()
    for(v <- values){
      if(v%div == 0){
        divisible = divisible ++ List(v)
      }
    }
    divisible
  }
